...
class PriorBox:
    def __init__(
        self,
        input_shape: tuple[int, int] = (320, 240),  # [in_w, in_h]
        output_shape: tuple[int, int] = (320, 240),  # [img_w, img_h]
    ...
    def decode(
        ...
        """Decodes the locations (x1, y1, x2, y2) and scores (c) from the priors, and
            the given loc and conf.
        Ignore low scores based on ignore_score.

        :param loc: loc produced from loc layers of shape [num_priors, 4].
            '4' for [x_c, y_c, w, h].
        :param conf: conf produced from conf layers of shape [num_priors, 2].
            '2' for [p_non_face, p_face].
        :param iou: iou produced from iou layers of shape [num_priors, 1]. '1' for [iou]
        :param ignore_score: used to filter out low score instances.
        :return: Detections, concatenated by bboxes, landmarks and scores.
            bboxes consists of num_priors * (x1, y1, x2, y2) of shape [num_priors, 4].
            Landmarks consists of:
            num_priors * (x_le, y_le, x_re, y_r2, x_n, y_n, x_ml, y_ml, x_mr, y_mr)
            of shape [num_priors, 5*2].
        """
        # get score
        cls_scores = conf[:, 1]
        iou_scores = iou[:, 0]
